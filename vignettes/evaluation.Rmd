...
# Download sensor data and annotations

The evaluation data is archived seperately from the R package. To download the data use download().
```{r}
download()
```
...
```{r,eval=FALSE}
results<-evaluate_image_crowns(submission = submission,project = T, show=F, summarize = T)
```
