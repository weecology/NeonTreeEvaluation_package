...
#' @export
stem_detection<-function(submission,project_boxes=FALSE, show=T){
  ...
  results<-submission %>% group_by(plot_name) %>% do(evaluate_stem(.,project_boxes=project_boxes,show=show,stem_dat=stem_dat))
}
